...
class SearchForm(forms.Form):
    keyword = forms.CharField(max_length=255, widget=forms.TextInput({'class': 'form-control', 'placeholder': u'名称'}))

class ComparisonForm(forms.Form):
    srcpackage = PackageSingleChoices(label=u'文件包一')
    destpackage = PackageSingleChoices(label=u'文件包二')
...
class TemplateTagValueForm(forms.Form):
    ...
    def clean_name(self):
        ...
        if not data.startswith('${') or not data.endswith('}$'):
            raise forms.ValidationError(u'模板标签名必须以 "${" 开始， 并以 "}$" 结束', code='invalid')
        if len(data) <= 4:
            raise forms.ValidationError(u'模板标签名不正确', code='invalid')
        trueTagName = data.replace('${', '').replace('}$', '')
        if not re.match('^[a-zA-Z0-9_]+$', trueTagName):
            raise forms.ValidationError(u'模板标签名仅允许大小写字母，数字，下划线', code='invalid')

        try:
            tag = models.TemplateTag.objects.get(name=data)
            if tag:
                raise forms.ValidationError(u'模板标签已存在！', code='invalid')
        except:
            pass
    ...
    def clean(self):
        ...
        for environment in environments:
            ...
            if models.TagValue.objects.filter(filter):
                raise forms.ValidationError(u'{0} 所对应的模板标签值已存在于环境 {1} 中！'.format(tagName, environment.name), code='invalid')

class ApplicationTagCopyForm(forms.Form):
    source = ApplicationSingleChoices(label=u'源应用程序', required=True)
    target = ApplicationSingleChoices(label=u'目标应用程序', required=True)
    ...
    def clean_source(self):
        ...
        if not appTags:
            raise forms.ValidationError(u'{0} 无关联应用标签， 无法进行复制！'.format(data.name), code='invalid')
    ...
    def clean_target(self):
        ...
        if appTags:
            raise forms.ValidationError(u'已有应用标签关联到 {0}， 无法进行复制！'.format(data.name), code='invalid')
    ...
    def clean(self):
        ...
        if source == target:
            raise forms.ValidationError(u'源应用程序和目标应用程序相同， 无法进行复制！', code='invalid')

class MoveApplicationForm(forms.Form):
    application = ApplicationSingleChoices(label=u'应用程序', required=True)
    department = DepartmentSingleChoices(label=u'所属部门', required=True)
    ...
    def clean(self):
        ...
        if isInDepartment:
            raise forms.ValidationError(u'该应用已关联到 {0}， 无法进行移动！'.format(department.name), code='invalid')
...
class ApplicationForm(forms.ModelForm):
    ...
    name = forms.CharField(label=u'应用程序名', max_length=255, widget=forms.TextInput({'class': 'form-control'}))
    description = forms.CharField(label=u'描述', required=False, max_length=255, widget=forms.TextInput({'class': 'form-control'}))
...
class DepartmentForm(forms.ModelForm):
    ...
    name = forms.CharField(label=u'部门名', max_length=255, widget=forms.TextInput({'class': 'form-control'}))
    description = forms.CharField(label=u'描述', required=False, min_length=0, max_length=255, widget=forms.TextInput({'class': 'form-control'}))
    applications = ApplicationMultipleChoices(label=u'应用列表', required=False)
...
class EnvironmentForm(forms.ModelForm):
    ...
    name = forms.CharField(label=u'环境名', max_length=255, widget=forms.TextInput({'class': 'form-control'}))
    description = forms.CharField(label=u'描述', required=False, max_length=255, widget=forms.TextInput({'class': 'form-control'}))
    ...
    def clean_name(self):
        ...
        if not re.match('^[a-zA-Z0-9_]+$', data):
            raise forms.ValidationError(u'环境名称仅允许大小写字母，数字，下划线', code='invalid')
...
class TemplateTagForm(forms.ModelForm):
    ...
    name = forms.CharField(label=u'模板标签名', max_length=255, widget=forms.TextInput({'class': 'form-control'}))
    description = forms.CharField(label=u'描述', max_length=255, required=False, widget=forms.TextInput({'class': 'form-control'}))
...
class ApplicationTagForm(forms.ModelForm):
    ...
    application = ApplicationSingleChoices(label=u'应用程序')
    file_path = forms.CharField(label=u'文件相对路径', max_length=255, widget=forms.TextInput({'class': 'form-control'}))
    tags = TemplateTagMultipleChoices(label=u'模板标签', required=False)
    ...
    def clean_file_path(self):
        ...
        if not data.startswith('/'):
            raise forms.ValidationError(u'文件路径必须使用以 / 开始的相对路径', code='invalid')
...
class TagValueForm(forms.ModelForm):
    ...
    environment = EnvironmentSingleChoices(label=u'关联环境')
    tag = TemplateTagSingleChoices(label=u'模板标签')
    value = forms.CharField(label=u'值', max_length=1024, widget=forms.TextInput({'class': 'form-control'}))
...
class AllTagValueForm(forms.Form):

    tag = TemplateTagSingleChoices(label=u'模板标签')
    value = forms.CharField(label=u'SIT1环境', max_length=1024, widget=forms.TextInput({'class': 'form-control'}))
    value1 = forms.CharField(label=u'SIT2环境', max_length=1024, widget=forms.TextInput({'class': 'form-control'}))
    value2 = forms.CharField(label=u'UAT环境', max_length=1024, widget=forms.TextInput({'class': 'form-control'}))
    value3 = forms.CharField(label=u'STAGING_PROD环境', max_length=1024, widget=forms.TextInput({'class': 'form-control'}))
...
class PackageForm(forms.ModelForm):
    ...
    application = ApplicationSingleChoices(label=u'应用')
    environment = EnvironmentSingleChoices(label=u'环境')
    branch_name = forms.CharField(label=u'GITLAB分支名称(区分大小写)', max_length=255, initial='master', required=False, widget=forms.TextInput({'class': 'form-control'}))
    target_platform = forms.ChoiceField(choices=models.Package.platforms, label=u'目标平台', widget=forms.Select({'class': 'form-control'}))
    description = forms.CharField(label=u'备注信息', max_length=255, required=False, widget=forms.TextInput({'class': 'form-control'}))
    ...
    def clean_environment(self):
        ...
        if self.cleaned_data['environment'].name == settings.PROD_ENV_NAME:
            if 'ops' not in groupnames:
                raise forms.ValidationError(u'只有运维团队可以打包生产环境的配置文件包！', code='invalid')
