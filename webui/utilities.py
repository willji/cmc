...
class ConfigurationHelper():
    ...
    def download_file(self, package):
        try:
            ...
            if not project:
                raise Exception(u'无法在Gitlab中找到项目，{0}'.format(package.application.name))
    ...
    def parse_file(self, package):
        try:
            ...
            # get certain files, then replace \n with \r\n
            if package.target_platform.lower().startswith('windows'):
                config_files = []
                pattern = re.compile('.txt|.config|.xml', re.IGNORECASE)
                for root, dirs, files in os.walk(dirPath):
                    for config_file in files:
                        if re.search(pattern, config_file):
                            config_files.append(os.path.join(root, config_file))

                for config_file in config_files:
                    with codecs.open(config_file, mode='rb', encoding='utf-8') as f:
                        content = f.read()
                        content = string.replace(content, '\n', '\r\n')

                    with codecs.open(config_file, mode='wb', encoding='utf-8') as f:
                        f.write(content)
